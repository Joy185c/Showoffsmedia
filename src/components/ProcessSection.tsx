...
const ProcessSection = () => {
  ...
  useEffect(() => {
    supabase.from("process_steps").select("*").order("sort_order").then(({ data }) => setSteps(data ?? []));
  }, []);

  return (
    <section id="process" className="py-20 px-4">
      <div className="max-w-5xl mx-auto">
        <p className="text-primary text-sm font-semibold uppercase tracking-wider text-center mb-3">our process</p>
        ...
        <div className="space-y-6">
          {steps.map((step, i) => (
            <motion.div key={step.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="glass-card p-6 md:p-8 flex flex-col md:flex-row items-start gap-6">
              <div className="flex items-center gap-4 md:min-w-[180px]">
                ...
                <span className="text-xs uppercase tracking-wider text-primary font-semibold bg-primary/10 px-3 py-1 rounded-full">{step.label}</span>
              </div>
              <div>
